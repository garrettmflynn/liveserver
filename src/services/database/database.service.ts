...
export class DatabaseService extends Service {
    ...
    mode: DatabaseMode = 'local' // TODO: Remove entirely so this isn't a global decision
    ...
    constructor (Router, dbOptions:{
        ...
        defaultCollections.forEach(k => {
            if (!dbOptions.collections[k])  dbOptions.collections[k] = {
                match: ['id']
            }
        })
    ...
    //             let auths = [];
    //             if(mode === 'mongo') {
    //                 let s = this.collections.authorizations.model.find({ $or:[{authorizedId: user.id},{authorizerId: user.id}] });
    //                 if(await s.count() > 0) {
    ...
    //         await this.collections.users.model.updateOne(toFind, {$set: copy}, {upsert: true}); 

    //         user = await this.collections.users.model.findOne(toFind);
    ...
    //         if(mode === 'mongo') {
    //             exists = await this.collections.groups.model.findOne({name:struct.name});
    //         } else {
    ...
    //         let ids = [];
    //         if(mode === 'mongo') {
    //             let cursor = this.collections.users.model.find({ $or: allusers }); //encryption references
    ...
    //                 await this.db.collection(struct.structType).insertOne(copy);
    //             }
    //             else await this.collections.groups.model.updateOne({ _id: safeObjectID(struct._id) }, {$set: copy}, {upsert: true}); 
    //         } else {
    //             this.setLocalData(struct);
    //         }
    ...
    //         let u = await this.collections.users.model.findOne({$or: query}); //encryption references
    ...
    //                 // console.log(u);
    //                 let authorizations = [];
    //                 let auths = this.collections.authorizations.model.find({ownerId:u.id});
    //                 if((await auths.count() > 0)) {
    //                     await auths.forEach(d => authorizations.push(d));
    //                 }
    //                 let gs = this.collections.groups.model.find({users:{$all:[u.id]}});
    //                 let groups = [];
    //                 if((await gs.count() > 0)) {
    ...
    //     let found = [];
    //     if (usrs.length > 0){
    //         let users = this.collections.users.model.find({$or:usrs});
    ...
    //     let users = this.collections.users.model.find({
    ...
    //     let auths = [];
    //     if(authId.length === 0 ) {
    //         let cursor = this.collections.authorizations.model.find({ownerId:ownerId});
    //         if(await cursor.count > 0) {
    ...
    //         }
    //     }
    //     else auths.push(await this.collections.authorizations.model.findOne({_id: safeObjectID(authId), ownerId:ownerId}));
    ...
    //     let groups = [];
    //     if(groupId.length === 0 ) {
    //         let cursor = this.collections.groups.model.find({users:{$all:[userId]}});
    //         if(await cursor.count > 0) {
    //             await cursor.forEach((a) => {
    //                 groups.push(a)
    //             });
    ...
    //     }
    //     else {
    //         try {groups.push(await this.collections.groups.model.findOne({_id:safeObjectID(groupId), users:{$all:[userId]}}));} catch {}
    //     }
    ...
    //             if(struct) {
    //                 structs.push(struct);
    //                 let notifications = await this.collections.notifications.model.find({parent:{structType:ref.structType,id:ref._id}});
    ...
    //     if(user.id !== userId) {
    //         let u = await this.collections.users.model.findOne({ id: userId });
    //         let passed = await this.checkAuthorization(user,u);
    //         if(!passed) return false;
    //     }

    //     await this.collections.users.model.deleteOne({ id: userId });
    ...
    //     let s = await this.collections.groups.model.findOne({ _id: safeObjectID(groupId) });
    //     if(s) {
    ...
    //         }
    //         await this.collections.groups.model.deleteOne({ _id:safeObjectID(groupId) });
    //         return true;
    //     } else return false; 
    // }
    ...
    //     let s = await this.collections.authorizations.model.findOne({ _id: safeObjectID(authId) });
    //     if(s) {
    //         if(user.id !== s.ownerId) {
    //             let passed = await this.checkAuthorization(user,s);
    //             if(!passed) return false;
    ...
    //             await this.collections.authorizations.model.deleteOne({ _id: safeObjectID(s.associatedAuthId) }); //remove the other auth too 
    ...
    //         }
    //         await this.collections.authorizations.model.deleteOne({ _id: safeObjectID(authId) });
    //         return true;
    //     } else return false; 
    // }
    ...
    //     if(mode === 'mongo'){
    //         let s = this.collections.authorizations.model.find(
    ...
    //                 if(this.mode.includes('mongo')) {
    //                     delete copy._id;
    //                     await this.collections.authorizations.model.updateOne({ $and: [ { authorizedId: authStruct.authorizedId }, { authorizerId: authStruct.authorizerId }, { ownerId: auth.ownerId } ] }, {$set: copy}, {upsert: true});
    //                 } else {
    ...
    //     if(mode ==='mongo') {
    //         delete copy._id;
    //         await this.collections.authorizations.model.updateOne({ $and: [ { authorizedId: authStruct.authorizedId }, { authorizerId: authStruct.authorizerId }, { ownerId: authStruct.ownerId } ] }, {$set: copy}, {upsert: true});
    //     } else {
    //         this.setLocalData(copy);
    //     }

    //     if(authStruct._id.includes('defaultId') && mode === 'mongo') {
    //         let replacedAuth = await this.collections.authorizations.model.findOne(copy);
    //         if(replacedAuth) {
    //             authStruct._id = replacedAuth._id.toString();
    //             if(otherAuthset) {
    //                 let otherAuth = await this.collections.authorizations.model.findOne({$and: [ { authorizedId: otherAuthset.authorizedId }, { authorizerId: otherAuthset.authorizerId }, { ownerId: otherAuthset.ownerId } ] });
    //                 if(otherAuth) {
    //                     otherAuth.associatedAuthId = authStruct._id;
    //                     delete otherAuth._id;
    //                     await this.collections.authorizations.model.updateOne({ $and: [ { authorizedId: otherAuth.authorizedId }, { authorizerId: otherAuth.authorizerId }, { ownerId: otherAuth.ownerId } ] }, {$set: otherAuth}, {upsert: true}); 
    ...
    //     let auth1, auth2;
    //     if(mode === 'mongo') {
    //         auth1 = await this.collections.authorizations.model.findOne({$or: [{authorizedId:user.id,authorizerId:struct.ownerId, ownerId:user.id},{authorizedId:struct.ownerId,authorizerId:user.id, ownerId:user.id}]});
    //         auth2 = await this.collections.authorizations.model.findOne({$or: [{authorizedId:user.id,authorizerId:struct.ownerId, ownerId:struct.ownerId},{authorizedId:struct.ownerId,authorizerId:user.id, ownerId:struct.ownerId}]});
    //     }
    //     else {
    ...
    // wipeDB = async () => {
    //     //await this.collections.authorizations.model.deleteMany({});
    //     //await this.collections.groups.model.deleteMany({});
    //     await Promise.all(Object.values(this.collections).map(c => c.model.deleteMany({})))
